...
propose changes to Sila JSON-RPC API.

## Introduction

The Sila JSON-RPC API is the canonical interface between users and the
Sila network. Each execution layer client implements the API as defined by
the spec.
...
protocol changes over time, new types of data become available. SIP-2930
necessitated the introduction of `sil_accessList` and SIP-1559 necessitated
`sil_feeHistory`.
...
without a protocol change. For example, `sil_getProof` has been possible since
the initial version of Sila -- yet, it was only standardized in recent years
as demand for the functionality grew. Before `sil_getProof`, there was no
...
change to the API. Although each client is able to validate the Sila chain,
there can be a huge variance in actual design decisions.

As an example, a proposal for a method such as `sil_totalSupply` seems
...
proposal is a PR to this repository ([sila/execution-apis][exec-apis]). A
...
- **All Core Devs Testing (ACDT) calls** — Recommended. Post a request on the
  AllCoreDevs agenda (usually in [sila/pm][pm]) to discuss the proposal.
...
3. **Implement in go-sila** — The spec must be implemented in go-sila
   ...
   rpctestgen uses go-sila libraries to produce the test artifacts;
   upstream go-sila changes may be required for new methods.
...
- **rpctestgen / test fill** — Required for existing methods. For **new
  methods** that require upstream go-sila changes, rpctestgen may not pass
  until go-sila implements the method. In such cases, maintainers may merge
...
- **Results** — Test results are published at [hive.silpandaops.io][hivetests]
...
[exec-apis]: https://github.com/sila-chain/execution-apis
[pm]: https://github.com/sila/pm
[test-gen]: ./tests.md
[wordlist]: https://github.com/sila-chain/execution-apis/blob/main/wordlist.txt
...
[workflows-readme]: https://github.com/sila-chain/execution-apis/blob/main/.github/workflows/README.md
