...
`Sil-Execution-Version: <fork>` request header.
...
require the `Sil-Execution-Version: <fork>` request header; diagnostic
endpoints ignore it.

| Resource | Endpoint | Purpose |
| - | - | - |
| Payload | `POST /engine/v1/payloads` | Submit a payload received from the CL gossip network for the EL to validate / import. Replaces `engine_newPayload`. Fork-scoped via `Sil-Execution-Version`. |
| Payload | `GET /engine/v1/payloads/{payloadId}` | Retrieve a built payload by id. Replaces `engine_getPayload`. Fork-scoped via `Sil-Execution-Version`. CL polls when it wants a fresher snapshot. |
| Forkchoice | `POST /engine/v1/forkchoice` | Atomic forkchoice update: update head/safe/finalized, optionally start a payload build, optionally update custody set. Replaces `engine_forkchoiceUpdated`. Fork-scoped via `Sil-Execution-Version`. |
| Bodies | `POST /engine/v1/bodies/hash` | Replaces `engine_getPayloadBodiesByHash`. `Sil-Execution-Version` selects both the response schema *and* the era of returned blocks; out-of-era blocks come back as `available=false`. |
...
- **Stale-fork header:** an FCU with `Sil-Execution-Version: <fork>`
  ...
  If `payload_attributes` is present, the `Sil-Execution-Version`
...
era of the returned blocks.** The `Sil-Execution-Version` header
...
- `POST /bodies/hash` with `Sil-Execution-Version: cancun` returns
...
- `POST /bodies/hash` with `Sil-Execution-Version: amsterdam` returns
...
different `Sil-Execution-Version` for the unfilled suffix.
...
- **Response body** (both endpoints): SSZ-encoded `BodiesResponse`
  ...
  serialised against the **schema named by `Sil-Execution-Version`**.
...
curl -X POST http://localhost:8551/engine/v1/payloads \
  ...
  -H "Sil-Execution-Version: amsterdam" \
...
Sil-Execution-Version: amsterdam
...
every fork — only the `Sil-Execution-Version` header value changes
...
# Sil-Execution-Version: osaka — identical envelope shape to Prague
...
| 400 Bad Request | `/engine-api/errors/unsupported-fork` | -38005 | `Sil-Execution-Version` value missing, unknown, or unsupported by this EL |
...
2. **Per-fork body schema** — selected via the
   `Sil-Execution-Version: <fork>` request header on hot-path
...
`Sil-Execution-Version` value. A BPO fork only changes blob-count
...
BPO1–BPO5 all send `Sil-Execution-Version: osaka` and use the Osaka
...
- **EL exposes `/engine/v1/...` but doesn't know the requested fork.**
  ...
  `Sil-Execution-Version: amsterdam` against an EL that only
...
- **Base path:** `/engine/v1/...`. `/v1` is the first major REST
  ...
  `Sil-Execution-Version: <fork>` request header rather than a URL
...
- **Fork header:** every hot-path request MUST carry
  `Sil-Execution-Version: <fork>`. Missing or unknown header on a
...
`Sil-Consensus-Version` for the same purpose on the CL side.
...
1. **Fork-scoped** (e.g. `/bodies`): `Sil-Execution-Version` selects
   ...
   ```
   # POST /bodies/hash with Sil-Execution-Version: amsterdam
   BodyEntry {
       available: boolean
   ...
   `Sil-Execution-Version: cancun` and receives the Cancun container
...
- Replacing `sil_*` JSON-RPC. The `sil` namespace stays JSON-RPC. This
...
- **No content negotiation.** A new fork structure forces a new method
  ...
  (`Sil-Execution-Version: amsterdam`), the body schema is selected
...
JSON-RPC is fine for the casual `sil_*` query API. For the hot path
...
8. **The split between `engine_*` namespace and the `sil_*` subset
   the EL must expose** — out of scope for this refactor; the `sil_*`
...
- **Backwards compatibility** is out of scope. The legacy JSON-RPC
  ...
  or forbid sunset.
- **`sil_*` JSON-RPC subset** (`sil_blockNumber`, `sil_call`,
  `sil_chainId`, `sil_getCode`, `sil_getBlockByHash`,
  `sil_getBlockByNumber`, `sil_getLogs`, `sil_sendRawTransaction`,
  `sil_syncing`) is **not** mirrored under `/engine/v1/...`. CLs that
...
- **Fork-scoped endpoints:** `/payloads`, `/forkchoice`, `/bodies`.
  Fork in the `Sil-Execution-Version` request header.
...
- **Cross-fork response containers** come in two flavours:
  fork-scoped (`/bodies`) uses `Sil-Execution-Version` to pick
...
- **Stale-fork header** is allowed when `payload_attributes` is
  ...
  `Sil-Execution-Version` MUST match the timestamp's fork
...
[SIP-7688](https://sips.sila.org/SIPS/sip-7688). Adopting
...
this spec once SIP-7688 is SFI-ed. At that point we can redesign
