...
| `VersionedHash` | `ByteVector[32]` | SIP-4844 versioned blob hash |
...
| `CELLS_PER_EXT_BLOB` | `128` | [SIP-7594](https://sips.sila.org/SIPS/sip-7594) |
| `FIELD_ELEMENTS_PER_CELL` | `64` | [SIP-7594](https://sips.sila.org/SIPS/sip-7594) |
| `BYTES_PER_CELL` | `FIELD_ELEMENTS_PER_CELL * BYTES_PER_FIELD_ELEMENT` (2,048) | [SIP-7594](https://sips.sila.org/SIPS/sip-7594) |
| `MAX_BAL_BYTES` | `MAX_BYTES_PER_TX` | [SIP-7928](https://sips.sila.org/SIPS/sip-7928) (placeholder until EIP pins a tighter bound) |
| `MAX_EXECUTION_REQUESTS_PER_PAYLOAD` | `2**8` (256) | [SIP-7685](https://sips.sila.org/SIPS/sip-7685) |
...
ExecutionPayload {
    ...
    block_access_list:  ByteList[MAX_BAL_BYTES]   # RLP-encoded SIP-7928 BAL
    slot_number:        Uint64
}
```
...
- `block_access_list` is RLP-encoded inside an SSZ `ByteList`. SIP-7928's
...
- `transactions` elements remain RLP-encoded `TransactionType ||
  TransactionPayload` per SIP-2718. Receiver-side rule: each element
...
`Sil-Execution-Version` request header. ELs handling
`Sil-Execution-Version: cancun` MUST use the Cancun containers; ELs
handling `Sil-Execution-Version: amsterdam` MUST use the Amsterdam
...
value of the `Sil-Execution-Version` request header. For instance
`POST /payloads` with `Sil-Execution-Version: cancun` takes an
`ExecutionPayloadEnvelopeCancun` wrapping an `ExecutionPayloadCancun`,
and `GET /payloads/{id}` with `Sil-Execution-Version: shanghai`
...
> field contributes neither an offset nor content. go-sila's
...
the `Sil-Execution-Version` request header per the catalogue.
...
Each `Sil-Execution-Version` value pairs with its own
`ExecutionPayloadBody` schema. The Amsterdam variant carries every
field unconditionally:

```
# Amsterdam ExecutionPayloadBody (Sil-Execution-Version: amsterdam)
...
```
# Cancun ExecutionPayloadBody (Sil-Execution-Version: cancun)
ExecutionPayloadBody {
    transactions: List[ByteList[MAX_BYTES_PER_TX], MAX_TXS_PER_PAYLOAD]
    withdrawals:  List[Withdrawal, MAX_WITHDRAWALS_PER_PAYLOAD]
}

# Paris ExecutionPayloadBody (Sil-Execution-Version: paris)
ExecutionPayloadBody {
    transactions: List[ByteList[MAX_BYTES_PER_TX], MAX_TXS_PER_PAYLOAD]
}
```
...
= `2048` ([SIP-7594](https://sips.sila.org/SIPS/sip-7594)). A cell
...
1. **`MAX_BAL_BYTES`.** SIP-7928 defines the BAL but hasn't pinned
   a numeric upper bound yet. The catalogue currently uses
   `MAX_BYTES_PER_TX` as a placeholder; this should be tightened
   once the EIP lands.
2. **`MAX_BYTES_PER_EXECUTION_REQUEST`.** SIP-7685 hasn't pinned a
